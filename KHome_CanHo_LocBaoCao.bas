...
Sub LocBaoCao()
    ...
    For i = 1 To UBound(sourceData, 1)
        If filterCondition = LCase(ALL_ITEMS_TEXT) Or LCase(CStr(sourceData(i, COL_DOT_TT))) = filterCondition Then
            ...
            resultsData(resultCounter, 2) = sourceData(i, 5)
            resultsData(resultCounter, 3) = sourceData(i, 6)
            resultsData(resultCounter, 4) = sourceData(i, 7)
            resultsData(resultCounter, 5) = sourceData(i, 8)
            resultsData(resultCounter, 6) = sourceData(i, 9)
            resultsData(resultCounter, 7) = sourceData(i, 16)
            resultsData(resultCounter, 8) = sourceData(i, 22)
            resultsData(resultCounter, 9) = sourceData(i, 20)
            resultsData(resultCounter, 10) = sourceData(i, 27)
            resultsData(resultCounter, 11) = sourceData(i, 28)
            resultsData(resultCounter, 12) = sourceData(i, 29)
            resultsData(resultCounter, 13) = sourceData(i, 25)
        End If
    Next i
